...
- The canonical suite additionally runs **stateful invariant campaigns** ([`contracts/v3/test/invariant/`](contracts/v3/test/invariant/)): fuzzed operation sequences with after-every-step assertions that lock finality, provenance immutability, exact revision accounting, metadata-hash replayability, supply conservation, and soulbound immobility all hold — and that forbidden operations revert with exactly the documented errors (`fail_on_revert` differential checking).
